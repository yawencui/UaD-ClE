...
def compute_confusion_matrix(tg_model, tg_feature_model, class_means, evalloader, print_info=False, device=None):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(evalloader):
            ...
            all_predicted.append(predicted.data.cpu().numpy())

            outputs_feature = np.squeeze(tg_feature_model(inputs).data.cpu().numpy())
            ...
            all_predicted_icarl.append(predicted_icarl.data.cpu().numpy())
            ...
            all_predicted_ncm.append(predicted_ncm.data.cpu().numpy())
